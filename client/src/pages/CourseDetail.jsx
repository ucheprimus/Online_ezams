...
const CourseDetail = () => {
  ...
  const fetchReviews = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/courses/${id}/reviews?page=${reviewsPage}&limit=${reviewsPerPage}`
      );
  ...
  // Pagination for curriculum sections
  const itemsPerPage = 5;
  const totalPages = course?.curriculum?.length ? Math.ceil(course.curriculum.length / itemsPerPage) : 1;
  const startIndex = (currentPage - 1) * itemsPerPage;
  const currentItems = course?.curriculum?.slice(startIndex, startIndex + itemsPerPage) || [];

  // Reviews pagination
  const totalReviewPages = Math.ceil((course?.reviews?.length || 0) / reviewsPerPage);
                ...
                <Tab eventKey="overview" title="Overview">
                  <div className="p-3 p-md-4">
                    ...
                    <div className="course-content">
                      ...
                      {course.curriculum && course.curriculum.length > itemsPerPage && (
                ...
                <Tab eventKey="reviews" title="Reviews">
                  <div className="p-3 p-md-4">
                    ...
                    <div className="reviews-list">
                      {(reviews.length > 0 ? reviews : course.reviews || []).map((review, index) => (
                      ...
                      {(!reviews.length && !course.reviews?.length) && (
            ...
            <Card className="enrollment-card border-0 shadow-sm mb-3">
              ...
              <Card.Body className="p-3 p-md-4">
                <div className="price-section text-center mb-3">
                  <h3 className="price fw-bold text-primary mb-2">
                ...
                >
                  {!course.isPublished ? 'Course Not Published' : 
